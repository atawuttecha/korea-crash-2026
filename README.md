...
**สิ่งที่กราฟแสดง:** สามแผงบนแกนเวลาเดียวกัน — KOSPI (มาตราส่วนลอการิทึม), Real GDP YoY แบบแท่ง และ CPI YoY กับอัตราการว่างงาน แถบสีคลุมช่วงพีคถึงจุดต่ำสุดของทั้ง 4 วิกฤต

**ข้อสรุปสำคัญ:** อัตราการว่างงานอยู่ที่ 2.75% (มิ.ย.) และ 2.80% (ก.ค.) เงินเฟ้ออยู่ที่ 3.16% และ 2.79% ตามลำดับ ใกล้เคียงระดับปกติ ส่วน GDP YoY ล่าสุด (เม.ย. 2026) ยังเป็นบวกที่ +3.75% ต่างจาก 3 วิกฤตในอดีตที่ดัชนีร่วงพร้อมเศรษฐกิจจริงชะลอตัวหรือหดตัว **สรุป:** การร่วงปี 2026 ไม่มีลักษณะของวิกฤตเศรษฐกิจมหภาคภายในประเทศ

> **ข้อควรระวัง:** GDP รายเดือนได้จากการ interpolate ข้อมูลรายไตรมาส ห้ามใช้พยากรณ์ และค่าล่าสุดคือเดือน เม.ย. 2026 ไม่ใช่ ณ วันเกิดเหตุ | **ที่มา:** KOSPI (yfinance) · Real GDP (FRED) · CPI (OECD SDMX) · Unemployment (FRED)
...
**สิ่งที่กราฟแสดง:** เส้น OECD CLI เทียบกับ CPI YoY แถบสีส้มระบายช่วงที่เข้าเงื่อนไข Stagflation (CLI < 100 และ CPI > 3% พร้อมกัน)
...
**สิ่งที่กราฟแสดง:** สามแผง — ① ราคาเบรนท์สกุลวอนแบบ YoY ② CPI YoY พร้อมเส้นเป้าหมาย 2% และเส้นเฝ้าระวัง 3% ③ ค่าสหสัมพันธ์ระหว่าง CPI กับราคาน้ำมันที่ lag ต่างๆ (2000–2026)

**ข้อสรุปสำคัญ:** ราคาเบรนท์กระโดดจาก 71.12 เป็น 109.72 ดอลลาร์ (+54% ภายในเดือนเดียว, ก.พ.→มี.ค. 2026) เงินเฟ้อทะลุ 3% ในเดือน พ.ค.–มิ.ย. ตรงกับช่วงจุดพีคของดัชนี ค่าสหสัมพันธ์สูงสุดที่ lag 2 เดือน (r = 0.52) แต่ค่า r ระหว่าง lag 1–7 เดือนใกล้เคียงกันมาก จึงสรุปได้เพียงว่า **ส่งผ่านภายในไม่กี่เดือน** อัตราดอกเบี้ยไม่เปลี่ยนแปลงตลอดช่วง → เงินเฟ้อจากฝั่งต้นทุนที่นโยบายการเงินไม่ได้ตอบสนอง

> **ข้อควรระวัง:** r ≈ 0.52 อธิบายความแปรปรวนได้เพียงราว 1 ใน 4 (r² ≈ 0.27) จึงเป็นปัจจัยร่วม ไม่ใช่สาเหตุเดียว ห้ามสรุปเรื่องอัตราดอกเบี้ยที่แท้จริงเพราะซีรีส์ที่มีเป็น Discount Rate | **ที่มา:** Brent และ USDKRW (FRED) · CPI (OECD SDMX) · Discount Rate (FRED INTDSRKRM193N)
...
| Real GDP | FRED (รายไตรมาส) | ใช้ได้จำกัด | Interpolation เป็นรายเดือน · ไม่มีข้อมูลตั้งแต่ พ.ค. 2026 · ห้ามใช้พยากรณ์ |
...
└── [ชุดข้อมูล .csv ที่ Notebook ดึงหรืออ้างอิง ถ้ามีการบันทึกไว้ใน repo]
```

> ถ้า Notebook ดึงข้อมูลสดจาก yfinance / FRED / OECD SDMX ทุกครั้งที่รัน (ไม่ได้เก็บ .csv ไว้ใน repo) สามารถตัดบรรทัดสุดท้ายออกและระบุไว้ในเอกสารว่า "ไม่มีไฟล์ข้อมูลดิบเก็บใน repo — ดึงสดทุกครั้งที่รัน Notebook" แทน
...
### 2. ติดตั้งแพ็กเกจ (รันในเซลล์แรกของ Notebook)

Colab มี Python และแพ็กเกจพื้นฐาน (pandas, numpy, matplotlib) ติดตั้งมาให้แล้ว ส่วนแพ็กเกจที่อาจต้องเพิ่มเอง:
...
Notebook จะดึงข้อมูลจาก yfinance / FRED / OECD SDMX สดจากอินเทอร์เน็ตตามค่าเริ่มต้น และสร้างกราฟทั้ง 12 ภาพใหม่ในเซสชัน Colab

### 4. ดาวน์โหลดกราฟกลับมาใส่ repo (ถ้าต้องการอัปเดต README)

รูปที่สร้างในเซสชัน Colab จะ**หายไปเมื่อปิดเซสชัน** เพราะเป็นพื้นที่ชั่วคราว หากต้องการให้กราฟ 12 ภาพที่ README อ้างอิงเป็นเวอร์ชันล่าสุด ให้เพิ่มโค้ดบันทึกไฟล์ในแต่ละเซลล์ที่ plot กราฟ แล้วดาวน์โหลดออกมาแทนที่ไฟล์เดิมใน repo:

```python
plt.savefig('fig1_macro_overview.png', dpi=150, bbox_inches='tight')
from google.colab import files
files.download('fig1_macro_overview.png')
```

หรือถ้าเชื่อม Colab กับ Google Drive/GitHub ไว้แล้ว ให้ commit ไฟล์ .png ที่อัปเดตกลับขึ้น repo ตำแหน่งเดิม (root ของ repo) เพื่อให้ลิงก์รูปใน README ยังใช้งานได้
